...
# Load a .env file only if it exists in the project (useful for local dev),
# but do not force loading a baked-in .env at runtime inside Docker images.
...
class AzureOpenAIService:
    ...
    def __init__(self):
        ...
        logger.info(
            f"System Message: {self.system_message[:50]}{'...' if len(self.system_message) > 50 else ''}"
        )

        # Validate required configuration
        if not self.endpoint or not self.api_key:
            logger.error(
                "Missing required Azure OpenAI configuration in environment variables"
            )
            raise ValueError(
                "Missing required Azure OpenAI configuration in environment variables"
            )
    ...
    def generate_response(
        ...
        try:
            ...
            payload = {"messages": messages, "max_completion_tokens": 800}
    ...
    def generate_response(
        ...
        if self.use_local_api:
            try:
                ...
                logger.info(
                    f"Sending request to local API with payload: {json.dumps(payload)}"
                )
                response = requests.post(
                    self.api_url,
                ...
                if response.status_code == 200:
                    response_data = response.json()
                    logger.info(
                        f"Received response from local API: {json.dumps(response_data)}"
                    )
            ...
            return self.azure_service.generate_response(
